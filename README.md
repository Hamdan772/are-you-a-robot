...
A fake CAPTCHA that turns boring identity verification into a strange, handmade browser game.
...
Open the page, check the box, and survive the verification process with five hearts.
...
- Mobile controls for keyboard-heavy levels, plus responsive grid and canvas sizing
- A five-heart health system with non-punitive logic puzzles where guessing should not cost lives
- Full-page flashlight challenge where the cursor becomes the only visible light
- LocalStorage save/resume with no backend
- A copyable final “Provisional Humanity Notice”
...
The app is built with React, TypeScript, Framer Motion, and plain CSS. Each level is a self-contained React component instead of a shared quiz template. That keeps the interactions flexible: one level can own a canvas, another can own keyboard movement, another can own a full-screen flashlight mask.
